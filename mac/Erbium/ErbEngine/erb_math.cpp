...
void vector::set(double xv, double yv, double zv, double wv)
{
  x=xv; y=yv; z=zv; w=wv;
}
...
bool vector::ascii_in(istream &in)
{
  char c;
  in >> c;
  if(c!='(') return false;
...
void vector::ascii_out(ostream &out)
{
...
double plane::classify(vector &p) const
{
  return (norm.dot((vector)p))+d; 
}

double plane::distance(vector &p) const
{
...
bool plane::ascii_in(istream &in)
{
  char c;
  in >> c;
  if(c!='(') return false;
  in >> norm.x >> norm.y >> norm.z >> d >> c;
  if(c!=')') return false;
  return true; 
}

void plane::ascii_out(ostream &out)
{
  out.precision(max_precision);
  out << "( " << norm.x << " " << norm.y
      << " " << norm.z << " " << d << " )";
