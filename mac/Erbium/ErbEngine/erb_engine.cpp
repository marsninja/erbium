...
//Polygon Class--------------------------------------------------------------
void polygon::add(const vector &pt)
{
  vert.add(pt);
}
...
void polygon::sort_verts()
{
  ...
  if(vsize<3) return;
  vector center; //First get center of polygon
...
vector &polygon::operator[](u_long i)
{
  return vert[i];
}
