...
struct box {vector orig; vector dim; bool valid; box():valid(false){}}; 
...
public:
  ...
  bag<vector> vert;  
public:
  void add(const vector &pt); //add a vector to polygon
  bool form_plane(); //use first 3 vectors to gen plane
...
public:
  vector &operator[](u_long i); //return vert[i]
