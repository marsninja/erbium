...
void snag_labels()
{
  ...
  { 
    ...
    { 
      ...
      {process_prms(1); cntr++;}
    else if(scan.token_text()=="render")
      {process_prms(1); cntr++;}
...
void assemble(const string &txt, ostream &out, ostream &er)
{
  ...
  while(scan.get_token())
  {
    ...
    {
        dummy.op=render; process_prms(1);
        dummy.ascii_out(out);
    }
    else if(scan.token_text()=="cuni")
    {
    ...
    else if(scan.token_text()=="seti")
    {
      ...
      get_imm(dummy.prm[0]); dummy.ascii_out(out);
    }
    else if(scan.token_text()=="add")
    {
    ...
    else if(scan.token_text()=="jsub")
    {
      ...
      {dummy.op=ret; dummy.ascii_out(out);}
    else if(scan.token_text()=="end")
      {dummy.op=end; dummy.ascii_out(out);}
  }
}
    
string assemble(const string &txt, ostream &er)
{
    ostringstream out;
    assemble(txt, out, er);
    return out.str();
