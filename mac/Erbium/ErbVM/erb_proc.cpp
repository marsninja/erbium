...
bag<construct> renderlist; //globally visible list of renderable objects
//Erbium Machine-------------------------------------------------------------
void erb_machine::trace(const u_long &ins) //located in erb_proc.cpp
{
  for(u_long i=0; i<ins; i++)
  {
    if((prog.inst[prog.cur]).op==end){break;}
    if(prog.at_end()){serror(no_end); break;}
    big_switch(prog.inst[prog.cur]); prog.cur++;
  }
}

void erb_machine::execute() //located in erb_proc.cpp
{
  //prog.ascii_out(cout); //JMK**
  while((prog.inst[prog.cur]).op!=end)
  {
    if(prog.at_end()){serror(no_end); break;}
    big_switch(prog.inst[prog.cur]); prog.cur++;
    //prog.inst[prog.cur].ascii_out(cout); //JMK**
  }
}
  ...
  switch(i.op)
  {
    ...
    case spit: //spit <type> <source> prints mem.mem.top().loc contents to screen 
      ...
      case spitmap: //spitmap <source> #only with constructs, prints quake map format
          ...
          break;
      case render: //render <source> #only constructs, adds to global renderlist
          if(i.prm[0]>=mem.mem.top().erb_elm.size())
            {serror(inval_prm); break;}
          renderlist.add(mem.mem.top().erb_elm[i.prm[0]]);
          break;
